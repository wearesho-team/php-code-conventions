...
Этот документ содержит правила написания кода (Code Conventions) в [компании Wearesho](http://roistat.com/ru/vacancy?roistat=github_codeconv). 
Здесь всегда находится актуальная версия нашего Code Conv, так как мы ссылаемся на него при проведении наших Code Review.

Оригинал написан в [компании Roistat](http://roistat.com/ru/vacancy?roistat=github_codeconv)
...
### 📖 Должна быть использована максимально возможная типизация для вашей версии PHP. Если версия PHP позволяет указать типы, без использование phpDoc, то его не нужно использовать
...
Если метод может быть использован наследниками класса, то он объявляется `protected`. Если используется сторонними классами, тогда `public`.
